namespace BCKFreightTMS.Web.ViewModels.Orders
{
    ...
    public class ListFinishedOrderViewModel : IMapFrom<Order>, IHaveCustomMappings
    {
        ...
        public void CreateMappings(IProfileExpression configuration)
        {
            configuration.CreateMap<Order, ListFinishedOrderViewModel>()
                .ForMember(x => x.Voyage, opt =>
                    opt.MapFrom(x => $"<p class='m-0'>{string.Join(" <i class='fas fa-angle-double-right'></i> ", x.OrderActions.OrderBy(oa => oa.TypeId).Select(oa => oa.Address.City))}</p>"));
        }
    }
}
